  ...
  return Object.create(events.EventEmitter.prototype, {
    ...
    join: {
      value: function(ws_connection) {
        var self = this;
        // In case of http long polling transport, when the user refreshes the browser, 
        // socket.io usually detects disconnection of the previous instance of the client
        // only well after the new client establishes a new connection and starts polling.
        // This late disconnect event results in the disconnection of the new client. 
        // Detaching an existing client from the session before connecting a new one mitigates this problem 
        // by ensuring only the latest client connection is associated with a debugging session. 
        var disconnect = function () {
          if (conn && !conn.debugSessionDetached) {
            // TODO what to do here? set timeout to close debugger connection
            conn.debugSessionDetached = true;
            self.emit('ws_closed');
            conn = null;
          }
        };
        ...
        conn.on('message', function (data) {
            if (!ws_connection.debugSessionDetached)
