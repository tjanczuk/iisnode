...
function getSession(debuggerPort) {
  ...
  if (!session) {
    ...
    // Not responding to the ws_closed event will retain a session in memory
    // event after a client disconnects (as a result of browser refresh or close).
    // Removing a session on that event prevented clients connecting over xhr-polling
    // from reconnecting to the debugged process.
    // TODO, tjaczuk, determine why this mechanism was required before 
...
DebugServer.prototype.start = function(options) {
  ...
  ws.configure(function() {
    ws.set('transports', config.transports);
    ws.set('log level', 1);
  });
